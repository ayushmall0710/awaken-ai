...
class OddballERPPipeline:
    ...
    def _process_session(
        ...
        try:
            ...
            try:
                # Get session_id from aligned_trials
                session_id = aligned_trials["session_id"].iloc[0]
            ...
            features = self._quantify_p300(
                ...
                n_standard_events_candidate=n_standard_events_candidate,
            )
    ...
    def _quantify_p300(
        ...
        Args:
            ...
            n_standard_events_candidate: Number of standard events found in aligned data (for diagnostics)
    ...
    def _build_clinical_table(self, patient_id: str, date: str, features: Dict[str, Any]) -> pd.DataFrame:
        ...
        Args:
            patient_id: Patient identifier
            date: Session date
            features: Feature dictionary from _quantify_p300
    ...
    def _build_electrode_detail_table(self, patient_id: str, date: str, features: Dict[str, Any]) -> pd.DataFrame:
        ...
        Returns:
            Three-row DataFrame (one per electrode: Fz, Cz, Pz)
        """
        rows = []
        for electrode in ["Fz", "Cz", "Pz"]:
            ...
            elif lat is None or np.isnan(lat):
    ...
    def _build_mapping_qc_table(self, patient_id: str, date: str, features: Dict[str, Any]) -> pd.DataFrame:
        ...
        Returns:
            Single-row DataFrame with mapping and QC metrics
        """
        return pd.DataFrame(
            [
                {
    ...
    def _update_master_feature_tables(
        ...
        Args:
            clinical_df: Table 1 (main analysis)
            detail_df: Table 2 (per-electrode detail)
            qc_df: Table 3 (mapping/QC diagnostics)
        """
        # Table 1: Clinical
    ...
    def _plot_topomap(self, diff_erp: mne.Evoked, patient_id: str, date: str):
        ...
        try:
            ...
            # MNE version here does not accept a custom `names` argument on
            # Evoked.plot_topomap, so we fall back to show_names=True (labels for
            # all electrodes) rather than selectively labeling Fz/Cz/Pz only.
