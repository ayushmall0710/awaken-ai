...
def _check_setup(patient_id: str, session: Optional[str], loader: UnifiedDataLoader) -> list[str]:
    ...
    sessions = [session] if session else loader.get_patient(patient_id).list_session_ids()
    if not any((config.EPOCHS_DIR / patient_id / session_id).exists() for session_id in sessions):
...
def run_cmd(
    ...
    # language specific
    report: bool = typer.Option(False, "--report", "-r", help="Generate HTML report after analysis."),
    # oddball specific
